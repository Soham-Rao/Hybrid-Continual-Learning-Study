...
def test_replay_buffer_load_state_forces_cpu_storage():
    buf = ReplayBuffer(capacity=10, strategy="reservoir")
    state = {
        "capacity": 10,
        "strategy": "reservoir",
        "storage": [
    ...
    assert len(buf) == 1
    assert buf._storage[0]["x"].device.type == "cpu"
    assert buf._storage[0]["y"].device.type == "cpu"
    assert buf._storage[0]["logits"].device.type == "cpu"
