...
class SplitMiniImageNet(BaseCLDataset):
    ...
    def __init__(
        ...
        self._train_base = datasets.ImageFolder(train_dir, transform=_build_transforms(True))

        if os.path.isdir(test_dir):
            self._test_base = datasets.ImageFolder(test_dir, transform=_build_transforms(False))
        else:
            # Fallback: 80/20 stratified split from training folder.
            base = datasets.ImageFolder(train_dir, transform=_build_transforms(False))
            train_idx, test_idx = self._stratified_split(base, seed)
            self._train_base = Subset(
                datasets.ImageFolder(train_dir, transform=_build_transforms(True)), train_idx
            )
