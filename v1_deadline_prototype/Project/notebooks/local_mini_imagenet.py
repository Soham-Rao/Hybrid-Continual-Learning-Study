"""Local Phase 4 runner for Split Mini-ImageNet.

This script is the local Phase 4 workflow entry point.
It defaults to:
- task-boundary resume enabled
- checkpoint cleanup after successful completion
- paths rooted in the current repository
"""
...
METHOD_OVERRIDES: Dict[str, Dict] = {
    ...
    "agem": {"buffer_size": 200, "agem_mem_batch": 64},
    "lwf": {"lwf_lambda": 1.0, "lwf_temp": 2.0},
    ...
    "er_ewc": {"buffer_size": 500, "ewc_lambda": 50.0},
    "progress_compress": {
        "pc_distill_w": 1.0,
        "pc_ewc_lambda": 100.0,
        "pc_compress_epochs": 3,
    },
    "agem_distill": {
        "buffer_size": 200,
        "agem_mem_batch": 64,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Split Mini-ImageNet locally.")
    parser.add_argument("--methods", default="all",
                        help="Comma-separated methods or 'all'.")
    parser.add_argument("--seeds", default="42",
                        help="Comma-separated seeds.")
    parser.add_argument("--device", default="cuda")
    parser.add_argument("--model", default="slim_resnet18",
                        choices=["slim_resnet18", "vit_small"])
    parser.add_argument("--data-root",
                        default=str(ROOT / "Project" / "data_local" / "mini-imagenet"))
    parser.add_argument("--log-dir",
                        default=str(ROOT / "results" / "phase4" / "local_mini" / "raw"))
    parser.add_argument("--figure-dir",
                        default=str(ROOT / "results" / "phase4" / "local_mini" / "figures"))
    parser.add_argument("--checkpoint-dir",
                        default=str(ROOT / "results" / "phase4" / "local_mini" / "checkpoints"))
    parser.add_argument("--batch-size", type=int, default=None)
    parser.add_argument("--lr", type=float, default=None)
    parser.add_argument("--num-workers", type=int, default=0)
    parser.add_argument("--max-samples-per-task", type=int, default=1000,
                        help="Only used by joint_training to keep local runs tractable.")
    ...
    for method_name in methods:
        ...
        if method_name == "joint_training":
            method_cfg["max_samples_per_task"] = args.max_samples_per_task

        for seed in seeds:
            try:
                print(f"\n=== split_mini_imagenet | {method_name} | seed={seed} ===")
    ...
    print("\nAll requested Mini-ImageNet runs finished successfully.")
