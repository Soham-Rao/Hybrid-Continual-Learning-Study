...
ViT-Small full sweeps are prioritized on Mini-ImageNet (Colab). Tiny-ImageNet uses top methods only due to free-tier GPU limits. This preserves the novelty claims while respecting hardware constraints.
**Always run with `disable_plots: false` and `disable_checkpoints: false`.**

## Phase 1 — Core Infrastructure ✅ COMPLETE (verified)
- [x] Project scaffold (`src/`, `experiments/`, `results/`, `notebooks/`, `app/`)
...
- [x] **Dataset loaders**
  ...
  - [x] Split Mini-ImageNet (Class-IL, 20 tasks × 5 classes) — *Colab notebook*
  - [x] Sequential Tiny-ImageNet (Class-IL, 10 tasks × 20 classes) — *Colab notebook*
- [x] **Model wrappers**
  - [x] Slim ResNet-18 (main local model, ~2M params)
  - [x] ResNet-8 (fast debug model)
  - [x] ViT-Small (Colab only, gradient checkpointing + FP16)
...
- [x] Colab notebooks: [colab_mini_imagenet.py](file:///d:/Catastrophic%20Forgetting/Project/notebooks/colab_mini_imagenet.py), [colab_tiny_imagenet.py](file:///d:/Catastrophic%20Forgetting/Project/notebooks/colab_tiny_imagenet.py)
...
- [x] FWT-corrected baseline rerun saved under `results_fwt/` (Permuted MNIST, Split CIFAR-10, Split CIFAR-100)

## Phase 3 — Run Hybrid Methods
- [x] Run all 7 hybrids on Permuted MNIST (FWT-enabled) → `results_phase3_fwt/`
...
- [x] SI-DER stability fix (unscale grads for SI accumulation) + Split CIFAR-100 full 5-seed rerun; results copied into main `results_phase3_fwt/` and fix logs kept in `results_phase3_fwt_sider_fix/`
...
- [x] Buffer-size ablation complete for Split CIFAR-10 DER (buf100/200/500) → `results_phase3_ablations/`
...
- [x] Statistical analysis (paired t-tests, Bonferroni, Cohen's d, confidence intervals) → `results_phase3_analysis/`
- [x] Generate Pareto frontier plots per dataset (see `results_phase3_fwt/figures/`)
- [x] Ensure configs exist for all dataset × method combos used in the study (incl. ablations)

## Phase 4 — Colab Experiments
- [ ] Mini-ImageNet: all methods, Slim ResNet-18, FP16, checkpoint to Drive
- [ ] Mini-ImageNet: ViT-Small all methods (gradient checkpointing, FP16)
- [ ] Tiny-ImageNet: top 3-4 hybrid methods with ViT-Small
- [ ] Download CSVs + PNGs → merge into master results
...
- [x] Paper-ready summary table (mean ± std) → `results_phase3_analysis/paper_ready_summary_pretty.csv`

## Phase 6 — Frontend (Streamlit Dashboard)
- [ ] Dashboard scaffold (`app/main.py`)
