...
class ReplayBuffer:
    ...
    def load_state_dict(self, state: Dict[str, Any]) -> None:
        ...
        for entry in raw_storage:
            x = entry["x"].detach().cpu()
            y = entry["y"].detach().cpu()
            ...
            if logits_cpu is not None:
                normalized["logit_size"] = int(entry.get("logit_size", logits_cpu.numel()))
            elif "logit_size" in entry:
                normalized["logit_size"] = entry["logit_size"]
...
class BaseCLMethod(ABC):
    ...
    def __init__(
        ...
        if self.device.type != "cuda":
            raise ValueError(
                "research_v2 execution is CUDA-only. "
                f"Received device={self.device!s}."
            )
    ...
    def autocast(self, enabled: Optional[bool] = None):
        """CUDA autocast helper for v2's CUDA-only execution policy."""
        if enabled is None:
            enabled = self.fp16
